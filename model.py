...
#read in the data of the data/IMG direction folder
dataDir = 'data/'
...
#generate train data and set the batchsize to 64
...
#generate validation data and set the batchsize also to 64
...
# we normalize and shuffle the images
def norm_image(img):
...
#The used Keras Sequential model, based on the NVIDIA paper
def define_model():
    model = Sequential()
...
# Let's save the model.json and model.h5 files in the working direction
...
# Assign the training and validation datasets to tr_gen and vl_gen with batchsize 64
tr_gen = train_gen(train_set, batchsize=64)
vl_gen = val_gen(val_set, batchsize=64)

#Run the model and save it when done training.
model = define_model()
model.fit_generator(generator=tr_gen, samples_per_epoch=len(steer_angle), nb_epoch=25, validation_data=vl_gen, nb_val_samples=64)
save_model(model)